...
- `develop` → `main` promotions are done manually (no automated `main` → `develop` sync job): open the PR yourself and merge it with a **merge commit**, for the same SHA-preservation reason as above.
