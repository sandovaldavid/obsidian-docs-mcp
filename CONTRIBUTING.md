...
- After every release-please release, a `sync-develop` job in `release-please.yml` automatically opens a `main` → `develop` PR (pre-built as a merge commit) so this doesn't have to be done by hand. It only opens the PR — merging is still manual, same as everything else in this repo.
