...
- PRs into `develop` merge via squash or rebase for regular feature/fix branches; PRs into `main` merge via squash or merge commit. Syncing `main` back into `develop` after a release uses a merge commit specifically, to keep commit ancestry intact.
